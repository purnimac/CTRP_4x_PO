
package org.iso._21090;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for QSD_TS.Date complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="QSD_TS.Date">
 *   &lt;complexContent>
 *     &lt;extension base="{uri:iso.org:21090}QSET_TS.Date">
 *       &lt;sequence>
 *         &lt;element name="first" type="{uri:iso.org:21090}QSET_TS.Date" minOccurs="0"/>
 *         &lt;element name="second" type="{uri:iso.org:21090}QSET_TS.Date" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/extension>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "QSD_TS.Date", propOrder = {
    "first",
    "second"
})
public class QSDTSDate
    extends QSETTSDate
{

    protected QSETTSDate first;
    protected QSETTSDate second;

    /**
     * Gets the value of the first property.
     * 
     * @return
     *     possible object is
     *     {@link QSETTSDate }
     *     
     */
    public QSETTSDate getFirst() {
        return first;
    }

    /**
     * Sets the value of the first property.
     * 
     * @param value
     *     allowed object is
     *     {@link QSETTSDate }
     *     
     */
    public void setFirst(QSETTSDate value) {
        this.first = value;
    }

    /**
     * Gets the value of the second property.
     * 
     * @return
     *     possible object is
     *     {@link QSETTSDate }
     *     
     */
    public QSETTSDate getSecond() {
        return second;
    }

    /**
     * Sets the value of the second property.
     * 
     * @param value
     *     allowed object is
     *     {@link QSETTSDate }
     *     
     */
    public void setSecond(QSETTSDate value) {
        this.second = value;
    }

    public QSDTSDate withFirst(QSETTSDate value) {
        setFirst(value);
        return this;
    }

    public QSDTSDate withSecond(QSETTSDate value) {
        setSecond(value);
        return this;
    }

    @Override
    public QSDTSDate withOriginalText(EDText value) {
        setOriginalText(value);
        return this;
    }

    @Override
    public QSDTSDate withNullFlavor(NullFlavor value) {
        setNullFlavor(value);
        return this;
    }

    @Override
    public QSDTSDate withFlavorId(String value) {
        setFlavorId(value);
        return this;
    }

    @Override
    public QSDTSDate withUpdateMode(UpdateMode value) {
        setUpdateMode(value);
        return this;
    }

    @Override
    public QSDTSDate withValidTimeLow(String value) {
        setValidTimeLow(value);
        return this;
    }

    @Override
    public QSDTSDate withValidTimeHigh(String value) {
        setValidTimeHigh(value);
        return this;
    }

    @Override
    public QSDTSDate withControlActRoot(String value) {
        setControlActRoot(value);
        return this;
    }

    @Override
    public QSDTSDate withControlActExtension(String value) {
        setControlActExtension(value);
        return this;
    }

}
